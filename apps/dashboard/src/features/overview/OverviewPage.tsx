...
export function OverviewPage() {
  ...
  const pending = transfers.filter((t) =>
    ['processing', 'screening', 'payout_submitted', 'paid_on_partner'].includes(t.status),
  ).length;
  ...
  return (
    <DashboardLayout title="Overview" sub="Money movement across live corridors, in real time.">
      ...
      >
        ...
        <div
          className="font-display font-extrabold tnum leading-none mt-2"
          style={{ fontSize: 56 }}
        >
          £{gbp0(totalVolume)}
        </div>
      ...
      <div className="flex items-center justify-between mt-8 mb-3">
        ...
        <Link
          to="/corridors"
          className="text-sm font-semibold text-[#FF5A2A] flex items-center gap-1"
        >
      ...
      <div className="flex gap-3 flex-wrap">
        {live.map((c) => (
          <div
            key={c.code}
            className="rounded-2xl bg-white border border-[#ECE5D8] px-4 py-3 flex items-center gap-3"
          >
            <span className="text-xl">{c.flag}</span>
            <div>
              <div className="text-sm font-semibold text-[#12233B]">{c.country}</div>
              <div className="text-xs tnum text-[#8A8578]">
      ...
      <div className="flex items-center justify-between mt-9 mb-3">
        ...
        <Link to="/transfers" className="text-sm font-semibold text-[#FF5A2A]">
          View all
        </Link>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="text-left text-[11px] uppercase tracking-wider text-[#B3AC9C]">
              <th className="font-semibold px-5 py-3">Transfer</th>
              <th className="font-semibold">Recipient</th>
              <th className="font-semibold">Corridor</th>
...
function HeroStat({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return (
    <div>
      <div
        className="font-display font-bold tnum text-3xl"
        style={{ color: accent ? '#FF9E7D' : '#fff' }}
      >
        {value}
      </div>
