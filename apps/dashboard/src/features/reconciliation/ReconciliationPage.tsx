  ...
  return (
    <DashboardLayout title="Reconciliation">
      <div className="grid grid-cols-3 gap-4 mb-4">
        <Card>
          <div className="text-xs text-[#9CA3AF]">Open breaks</div>
          <div className="font-display font-extrabold text-2xl text-[#12233B] mt-1">
            {breaks.length}
          </div>
        </Card>
        <Card>
          <div className="text-xs text-[#9CA3AF]">Oldest</div>
          <div className="font-display font-extrabold text-2xl text-[#12233B] mt-1">
            {Math.max(...breaks.map((b) => b.ageHours))}h
          </div>
        </Card>
        <Card>
          <div className="text-xs text-[#9CA3AF]">Partners affected</div>
          <div className="font-display font-extrabold text-2xl text-[#12233B] mt-1">
            {new Set(breaks.map((b) => b.partner)).size}
          </div>
        </Card>
      </div>
      ...
      <Card>
        ...
        <p className="text-sm text-[#9CA3AF] mb-4">
          Nothing settles to the ledger until matched against a partner statement line.
        </p>
        <table className="w-full text-sm">
          <thead className="text-[#9CA3AF] text-left">
            <tr>
              <th className="font-medium py-2">Break</th>
              <th className="font-medium">Transfer</th>
              <th className="font-medium">Partner</th>
          ...
          <tbody>
            {breaks.map((b) => (
              <tr key={b.id} className="border-t border-[#f3f3f3]">
                ...
                <td className="text-[#12233B] font-semibold">
                  {(b.amountMinor / 100).toLocaleString('en-GB', { minimumFractionDigits: 2 })}{' '}
                  {b.ccy}
                </td>
                <td className="text-[#6B7280]">{b.ageHours}h</td>
                <td>
                  <button className="text-xs font-semibold text-[#FF5A2A]">Resolve</button>
                </td>
              </tr>
            ))}
          </tbody>
