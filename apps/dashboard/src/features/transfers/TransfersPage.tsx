...
const fmt = (m: number, ccy: string) =>
  `${(m / 100).toLocaleString('en-GB', { minimumFractionDigits: 2 })} ${ccy}`;
      ...
      transfers.filter(
        (t) =>
          ...
          (q === '' ||
            ...
            t.recipient.toLowerCase().includes(q.toLowerCase())),
      ),
    [filter, q],
  );
  ...
  return (
    <DashboardLayout title="Transfers">
      <Card>
        ...
        <table className="w-full text-sm">
          <thead className="text-[#9CA3AF] text-left">
            <tr>
              ...
              <th className="font-medium">Corridor</th>
              <th className="font-medium">You send</th>
              <th className="font-medium">Receives</th>
        ...
        {rows.length === 0 ? (
          <div className="text-center text-[#9CA3AF] py-10 text-sm">No transfers match.</div>
        ) : null}
      </Card>
    </DashboardLayout>
  );
