  ...
  return (
    ...
    >
      <div className="px-6 py-6 flex items-center gap-3">
        <div
          className="h-9 w-9 rounded-xl flex items-center justify-center font-display font-extrabold"
        ...
        {NAV.map(({ to, label, icon: Icon, end }) => (
          ...
          >
            {({ isActive }) => (
              <>
                {isActive && (
                  <span className="absolute left-0 top-1/2 -translate-y-1/2 h-5 w-1 rounded-r bg-[#FF5A2A]" />
                )}
